...
# Se ejecuta desde ~/bbrna. Depende de s07 (usa salida/faa/kc583.faa).
set -u
cd ~/bbrna
...
mkdir -p salida/che && cd salida/che
...
echo "Genoma: $G"
python3 - "$G" <<'PY' > salida/utr_flagelina.fa
import sys
...
PY
$RNAFOLD --noPS < salida/utr_flagelina.fa | tee salida/utr_mfe.txt

python3 - "$G" <<'PY' > salida/control.fa
import sys,random
random.seed(7)
...
PY
$RNAFOLD --noPS < salida/control.fa | grep -oP '\(\s*-?[0-9]+\.[0-9]+\)$' | tr -d '() ' > salida/mfe_ctrl.txt
python3 -c "
import statistics as st
v=sorted(float(x) for x in open('salida/mfe_ctrl.txt'))
u=float(open('salida/utr_mfe.txt').read().strip().split('(')[-1].rstrip(')').strip())
