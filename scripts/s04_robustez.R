...
setwd("~/bbrna")
dds <- readRDS("v2/dds_v2.rds"); ddsY <- readRDS("v2/ddsY_v2.rds")
rG<-read.csv("v2/res_rG.csv",row.names=1); rY<-read.csv("v2/res_rY.csv",row.names=1)
r30<-read.csv("v2/res_r30.csv",row.names=1); r33<-read.csv("v2/res_r3730.csv",row.names=1)
rp<-read.csv("v2/res_rpH.csv",row.names=1); wp<-rG$wp

map<-read.delim("v2/lt2prot.tsv",header=FALSE); names(map)<-c("lt","prot")
fl <-read.delim("v2/flag_gff.tsv",header=FALSE); names(fl)<-c("lt","ini","fin","hebra","prod")
...
write.csv(tb,"v2/chk_conteos.csv",row.names=FALSE)
...
write.csv(est,"v2/chk_estricto.csv",row.names=FALSE)
...
write.csv(yl,"v2/chk_yale.csv",row.names=FALSE)
...
write.csv(c31,"v2/chk_curva31.csv",row.names=FALSE)
...
print(ef); write.csv(ef,"v2/chk_especificidad.csv",row.names=FALSE)
...
if (length(i25)>=2 && length(i37)>=2){
  ...
  write.csv(pt,"v2/chk_particiones.csv",row.names=FALSE)
}

mn <- sapply(c("Pl25","Pl30","Pl37"), function(k) round(rowMeans(nc[,cn==k,drop=FALSE])))
lk <- data.frame(wp=wp, lt=map$lt[match(wp,map$prot)])
lk$prod <- read.delim("v2/cds_todos.tsv",header=FALSE)[,5][match(lk$lt, read.delim("v2/cds_todos.tsv",header=FALSE)[,1])]
ab <- cbind(lk, mn)
cat("\n--- 15 MAS ABUNDANTES A 37 C ---\n")
print(head(ab[order(-ab$Pl37), c("lt","prod","Pl25","Pl30","Pl37")], 15))
write.csv(head(ab[order(-ab$Pl37),],15), "v2/chk_ranking.csv", row.names=FALSE)
write.csv(head(ab[order(-rowMeans(mn)),],15), "v2/chk_top_global.csv", row.names=FALSE)
