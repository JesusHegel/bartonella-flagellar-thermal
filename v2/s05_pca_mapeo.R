setwd("~/bbrna")
p <- read.csv("v2/pca_v2.csv", row.names=1); p$run <- rownames(p)
m <- read.delim("v2/mapeo.tsv", header=FALSE); names(m) <- c("run","mapeo")
...
write.csv(d, "v2/chk_pca_mapeo.csv", row.names=FALSE)
